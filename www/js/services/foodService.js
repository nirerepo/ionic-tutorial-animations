function FoodService($q, $http, Connection) {
    this.plateByName = function(queryText) {
        var rdo = $q.defer();
        var query = "http://nire0.gailen.es:9200/platos_es_co/_search?q=nombredieta:" + queryText + "&fields=id,nombredieta,kcal,cantidad,tipo_item,medida_casera,cantidad_medida_casera,grupo,saludable&sort=_score:desc,relevancia:asc";
        $http.get(query)
            .then(function (res) {
            rdo.resolve(res);
